...
func (h *Handler) APIEnqueue(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		negotiate.Error(w, r, http.StatusBadRequest, "JSONの解釈に失敗しました: "+err.Error())
		return
	}
	...
	default:
		negotiate.Error(w, r, http.StatusBadRequest, fmt.Sprintf("command は %q / %q / %q です",
	...
	if body.MusicJobID != "" {
		...
		if rErr != nil {
			negotiate.Error(w, r, http.StatusBadRequest, rErr.Error())
			return
		}
		inputURI = resolved
	}
	...
	if err != nil {
		negotiate.Error(w, r, http.StatusInternalServerError, "ジョブIDの発行に失敗しました")
		return
	}
	...
	if command == domain.CommandSynthesize {
		if body.Script == nil {
			negotiate.Error(w, r, http.StatusBadRequest, "synthesize には script が要ります")
			return
		}
		cleaned, vErr := h.validateScript(*body.Script)
		if vErr != nil {
			negotiate.Error(w, r, http.StatusBadRequest, vErr.Error())
			return
		}
		if saveErr := h.repo.SaveScript(r.Context(), jobID, cleaned); saveErr != nil {
			negotiate.Error(w, r, http.StatusBadGateway, "台本の保存に失敗しました")
			return
		}
	}
...
func (h *Handler) APIJobStatus(w http.ResponseWriter, r *http.Request) {
	...
	case errors.Is(err, jobstatus.ErrNotFound):
		negotiate.Error(w, r, http.StatusNotFound, "ジョブ状態が見つかりません")
		return
	case err != nil:
		slog.ErrorContext(r.Context(), "ジョブ状態の取得に失敗しました", "job_id", jobID, "error", err)
		negotiate.Error(w, r, http.StatusBadGateway, "ジョブ状態を読めませんでした")
		return
	}
...
func (h *Handler) APIPreviewReading(w http.ResponseWriter, r *http.Request) {
	...
	if len(body.Lines) == 0 {
		negotiate.Error(w, r, http.StatusBadRequest, "lines が空です")
		return
	}
	if len(body.Lines) > maxScriptLines {
		negotiate.Error(w, r, http.StatusBadRequest,
